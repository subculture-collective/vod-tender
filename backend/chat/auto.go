...
func StartAutoChatRecorder(ctx context.Context, db *sql.DB) {
    ...
    if channel == "" { slog.Info("auto chat: TWITCH_CHANNEL empty; abort"); return }
    if os.Getenv("TWITCH_BOT_USERNAME") == "" { slog.Info("auto chat: TWITCH_BOT_USERNAME empty; abort"); return }
    clientID := os.Getenv("TWITCH_CLIENT_ID")
    clientSecret := os.Getenv("TWITCH_CLIENT_SECRET")
    if clientID == "" || clientSecret == "" { slog.Info("auto chat: missing client id/secret; abort"); return }

    pollEvery := 30 * time.Second
    if v := os.Getenv("CHAT_AUTO_POLL_INTERVAL"); v != "" {
        if d, err := time.ParseDuration(v); err == nil && d > 0 { pollEvery = d }
    }
    ...
    if v := os.Getenv("VOD_RECONCILE_DELAY"); v != "" {
        if d, err := time.ParseDuration(v); err == nil && d > 0 { reconcileDelay = d }
    }
    ...
    for {
        ...
        func() {
            ...
            if err != nil { slog.Debug("auto chat: get app token", slog.Any("err", err)); return }
            ...
            if err != nil { slog.Debug("auto chat: streams req", slog.Any("err", err)); return }
            defer resp.Body.Close()
            var body struct { Data []struct { Title string `json:"title"`; StartedAt time.Time `json:"started_at"` } `json:"data"` }
            if err := json.NewDecoder(resp.Body).Decode(&body); err != nil { slog.Debug("auto chat: decode", slog.Any("err", err)); return }
            if len(body.Data) == 0 {
                // Offline
                if running && !reconciled {
                    // Stop current recorder if not already stopped
                    if recCancel != nil { recCancel() }
                    offStarted := time.Now()
                    ...
                    go func(ph string, st time.Time, offAt time.Time) {
                        ...
                        for attempt := 0; attempt < 30; attempt++ { // roughly up to reconcileWindow depending on delay
                            if ctx.Err() != nil { return }
                            if time.Since(offAt) > reconcileWindow { slog.Warn("auto chat: reconciliation window expired", slog.String("placeholder_vod", ph)); return }
                            ...
                            if err == nil {
                                ...
                                for i := range vods {
                                    v := vods[i]
                                    if v.Date.Before(st.Add(-10*time.Minute)) || v.Date.After(st.Add(10*time.Minute)) { continue }
                                    if candidate == nil {
                                        candidate = &v
                                ...
                                if candidate != nil {
                                    tx, err := db.Begin()
                                    if err != nil { slog.Warn("auto chat: reconcile begin tx", slog.Any("err", err)); return }
                                    // Ensure real VOD row; then refresh metadata (title/date/duration)
                                    _, _ = tx.Exec(`INSERT OR IGNORE INTO vods (twitch_vod_id, title, date, duration_seconds, created_at) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)`, candidate.ID, candidate.Title, candidate.Date, candidate.Duration)
                                    _, _ = tx.Exec(`UPDATE vods SET title=?, date=?, duration_seconds=?, updated_at=CURRENT_TIMESTAMP WHERE twitch_vod_id=?`, candidate.Title, candidate.Date, candidate.Duration, candidate.ID)
                                    ...
                                    if !candidate.Date.Equal(st) {
                                        ...
                                        if _, err := tx.Exec(`UPDATE chat_messages SET rel_timestamp=rel_timestamp - ? WHERE vod_id=?`, delta, ph); err != nil { _ = tx.Rollback(); slog.Warn("auto chat: reconcile shift timestamps", slog.Any("err", err)); return }
                                    }
                                    if _, err := tx.Exec(`UPDATE chat_messages SET vod_id=? WHERE vod_id=?`, candidate.ID, ph); err != nil { _ = tx.Rollback(); slog.Warn("auto chat: reconcile update chat", slog.Any("err", err)); return }
                                    if _, err := tx.Exec(`DELETE FROM vods WHERE twitch_vod_id=?`, ph); err != nil { _ = tx.Rollback(); slog.Warn("auto chat: reconcile delete placeholder", slog.Any("err", err)); return }
                                    if err := tx.Commit(); err != nil { slog.Warn("auto chat: reconcile commit", slog.Any("err", err)); return }
            ...
            reconciled = false
            _, _ = db.Exec(`INSERT OR IGNORE INTO vods (twitch_vod_id, title, date, duration_seconds, created_at) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)`, placeholder, "LIVE: "+body.Data[0].Title, startedAt, 0)
            running = true
