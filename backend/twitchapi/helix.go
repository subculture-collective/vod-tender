...
func (hc *HelixClient) http() *http.Client { if hc.HTTPClient != nil { return hc.HTTPClient }; return http.DefaultClient }
...
func (hc *HelixClient) GetUserID(ctx context.Context, login string) (string, error) {
	if login == "" { return "", fmt.Errorf("login empty") }
	...
	q := req.URL.Query(); q.Set("login", login); req.URL.RawQuery = q.Encode()
	...
	if len(body.Data) == 0 { return "", fmt.Errorf("user not found") }
...
func (hc *HelixClient) ListVideos(ctx context.Context, userID, after string, first int) ([]VideoMeta, string, error) {
	if userID == "" { return nil, "", fmt.Errorf("userID empty") }
	if first <= 0 { first = 20 }
	tok, err := hc.AppTokenSource.Get(ctx)
	if err != nil { return nil, "", err }
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.twitch.tv/helix/videos", nil)
	q := req.URL.Query(); q.Set("user_id", userID); q.Set("type","archive"); q.Set("first", fmt.Sprintf("%d", first)); if after!="" { q.Set("after", after) }; req.URL.RawQuery = q.Encode()
	req.Header.Set("Client-Id", hc.ClientID)
	req.Header.Set("Authorization", "Bearer "+tok)
	resp, err := hc.http().Do(req)
	if err != nil { return nil, "", err }
	defer resp.Body.Close()
	var body struct { Data []struct { ID, Title, Duration string; CreatedAt string `json:"created_at"` } `json:"data"`; Pagination struct { Cursor string `json:"cursor"` } `json:"pagination"` }
	if err := json.NewDecoder(resp.Body).Decode(&body); err != nil { return nil, "", err }
	out := make([]VideoMeta, 0, len(body.Data))
	for _, v := range body.Data { out = append(out, VideoMeta{ID:v.ID, Title:v.Title, Duration:v.Duration, CreatedAt:v.CreatedAt}) }
