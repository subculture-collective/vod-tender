...
func FetchAllChannelVODs(ctx context.Context, db *sql.DB, maxCount int, maxAge time.Duration) ([]VOD, error) {
	...
	cutoff := time.Time{}
	if maxAge > 0 { cutoff = time.Now().Add(-maxAge) }
	pageSize := 100
	if maxCount > 0 && maxCount < pageSize { pageSize = maxCount }
	after := ""
	if maxAge == 0 { _ = db.QueryRowContext(ctx, `SELECT value FROM kv WHERE key='catalog_after'`).Scan(&after) }
	collected := []VOD{}
	for {
		if maxCount > 0 && len(collected) >= maxCount { break }
		...
		for _, v := range videos {
			...
			if !cutoff.IsZero() && vodObj.Date.Before(cutoff) { return collected, nil }
...
func StartVODCatalogBackfillJob(ctx context.Context, db *sql.DB) {
	...
	if v := os.Getenv("VOD_CATALOG_BACKFILL_INTERVAL"); v != "" { if d, err := time.ParseDuration(v); err == nil && d > 0 { interval = d } }
	maxCount := 0
	...
	for {
		select {
		case <-ctx.Done(): slog.Info("catalog backfill job stopped"); return
		case <-ticker.C:
			if err := BackfillCatalog(ctx, db, maxCount, maxAge); err != nil { slog.Warn("catalog backfill", slog.Any("err", err)) }
		}
	}
}
...
func parseTwitchDuration(s string) int {
	...
	cur := ""
	for _, r := range s {
		if r >= '0' && r <= '9' { cur += string(r); continue }
		if cur == "" { continue }
		n := 0
		for _, d := range cur { n = n*10 + int(d-'0') }
		switch r {
		case 'h': total += n * 3600
		case 'm': total += n * 60
		case 's': total += n
