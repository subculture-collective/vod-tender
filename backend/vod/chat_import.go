...
func ImportChat(ctx context.Context, db *sql.DB, vodID string) error {
    ...
    if err != nil { return fmt.Errorf("prepare insert chat: %w", err) }
    defer stmt.Close()

    // Iterate over offsets
    step := 30 // seconds per page
    maxOffset := durationSeconds
    if maxOffset <= 0 { maxOffset = 24 * 60 * 60 } // cap at 24h when unknown
    emptyStreak := 0
    ...
    msgs, nextOffset, err := fetchRechatChunk(ctx, vodID, offset, cookieHeader)
        ...
        for _, m := range msgs {
            if m.ID != "" {
                if _, ok := seenIDs[m.ID]; ok { continue }
                seenIDs[m.ID] = struct{}{}
            }
            abs := m.Abs
            if abs.IsZero() {
...
func doFetchRechat(ctx context.Context, urlStr string, offset int, cookieHeader string) ([]rechatMessage, int, error) {
    ...
    for _, d := range raw.Data {
        ...
        if user == "" { user = a.Message.User.UserLogin }
        rel := a.Offset
        if rel == 0 {
...
func buildTwitchCookieHeader() string {
    ...
    if path == "" { path = "/run/cookies/twitch-cookies.txt" }
    fi, err := os.Stat(path)
